 ...
 * </p>
 * <p>
 * Description: Represent the cached information for a concept or an individual. For concepts this represents the root node of the tableau completion graph
 ...
 * node for concepts may be incomplete if the satisfiability status was cached when the satisfiability of another concept was being computed. Incomplete cached
...
public interface CachedNode
{
	...
	/**
	 * Checks if this node is connected to another node with the given role (or one of its subproperties). The node may have an incoming edge with the inverse
	...
	/**
	 * Returns if this node was cached without any dependency to a non-deterministic branch. In the presence of nominals, when we are checking the
	 * satisfiability of a concept the root node may be merged to a nominal node and that merge may be due to a non-deterministic branch. In such cases the
	 ...
	 *
	 * @return If this node was cached without any dependency to a non-deterministic branch
	 */
